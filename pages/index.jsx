...
const Home = () => {
  ...
  const fetchNFTs = async() => {
    ...
    const api_key = "rl7GCQyKFyC8XW7jaWKNIuGWChvxOqZO";
    ...
    if (!collection.length) {
      ...
      const fetchURL = `${baseURL}?owner=${wallet}&startToken=${tokenId}`;
      nfts = await fetch(fetchURL, requestOptions).then(data => data.json());
    } else {
      console.log("fetching nfts for collection owned by address")
      const fetchURL = `${baseURL}?owner=${wallet}&contractAddresses%5B%5D=${collection}&startToken=${tokenId}`;
      nfts= await fetch(fetchURL, requestOptions).then(data => data.json());
    }
  ...
  const fetchNFTsForCollection = async () => {
    if (collection.length) {
      ...
      const fetchURL = `${baseURL}?contractAddress=${collection}&withMetadata=${"true"}&startToken=${tokenId}`;
